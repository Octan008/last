...
class NeRFNetwork(NeRFRenderer):
    def __init__(self,
        ...
        self.sigma_net2 = FFMLP(
            input_dim=self.in_dim*2, 
    ...
    def density(self, x):
        ...
        x = self.encoder(x, bound=self.bound)
        # print("x-shape", x.shape, torch.cat([x.clone(),x.clone()], dim=1).shape)
        h = self.sigma_net(x)
        h2 = self.sigma_net2(torch.cat([x.clone(),x.clone()], dim=1))
        print("h2", h2.shape)
    ...
    def color(self, x, d, mask=None, geo_feat=None, **kwargs):
        ...
        if mask is not None:
            ...
            rgbs = h
        # mask = mask_point(h, [0.1, 0.1, 0.1], 0.05)
        # rgbs[mask] = torch.tensor([1.0, 0.0, 0.0], dtype=torch.float16, device=torch.device('cuda:0'))
