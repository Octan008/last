...
class NeRFSkeletonDataset(Dataset):
    def __init__(self, path, type='train', mode='colmap', preload=False, downscale=1, scale=0.33, n_test=10):
        ...
        if mode == 'colmap':
            with open(os.path.join(path, 'transforms_animation.json'), 'r') as f:
                transform = json.load(f)
        elif mode == 'blender':
            # load all splits (train/valid/test), this is what instant-ngp in fact does...
            if type == 'all':
                # transform_paths = glob.glob(os.path.join(path, '*.json'))
                transform_paths = glob.glob(os.path.join(path, 'transforms_animation.json'))
                transform = None
        ...
        if mode == 'colmap' and type == 'test':
            ...
            for f in frames:
                ...
                self.frame_ids.append(f["anim_frame"])
                if type == "val" or True:
                    self.frame_pose.append(f["animation"])

        self.poses = np.stack(self.poses, axis=0)
        tmp_frame_map = sorted(set(self.frame_ids))
        self.frame_map={}
        for i, fm in enumerate(tmp_frame_map):
            self.frame_map[fm] = i
    ...
    def num_pose(self):
        return len(list(self.frame_map.keys()))
    def get_pose_by_frame(self, index):
        return{
            "frame_id" :  self.frame_map[self.frame_ids[index]],
            "animation_pose" : self.frame_pose[index]
        }
    ...
    def __getitem__(self, index):
        ...
        if self.type == "val" or True:
            results["frame_pose"] = self.frame_pose[index]
