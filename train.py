...
def skeleton_optim(rank, args, n_gpu=1):
    ...
    train_dataset = dataset(args.datadir, split='train',
                            downsample=args.downsample_train, is_stack=True, data_preparation=True)
    test_dataset = dataset(args.datadir, split='train',
                           downsample=args.downsample_train, is_stack=True, data_preparation=True)
    ...
    N_voxel_list = (torch.round(torch.exp(torch.linspace(np.log(args.N_voxel_init), np.log(
        args.N_voxel_final), len(upsamp_list)+1))).long()).tolist()[1:]
    ...
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=None, pin_memory=True, num_workers=4)
    test_dataloader = torch.utils.data.DataLoader(
        test_dataset, batch_size=None, pin_memory=True, num_workers=4)
    print(w, h)
    ...
    pbar = tqdm(range(args.n_iters),
                miniters=args.progress_refresh_rate, file=sys.stdout)
    ...
    skeleton_dataset = LearnSkeletonPose(num_animFrames, len(
        joints), type=args.pose_type, use_tail=args.free_opt7)
    # para_pos
    if args.free_opt4:
        skeleton.refresh()
        tfs = skeleton.get_listed_global_transforms()
        translates = tfs[..., :3, 3]
        assert (not torch.isnan(tfs).any())
        rotations = skeleton.matrix_to_euler_pos_batch(
            tfs[..., :3, :3], top_batching=True)
        # print(rotations.shape, translates.shape)
        # exit()
        assert (not torch.isnan(rotations).any())
        para_six_pose = torch.cat([rotations, translates], dim=-1)
        skeleton_dataset = LearnSkeletonPose(num_animFrames, len(
            joints), type="para_six", para_pose=para_six_pose)

    skeleton_dataset.to(device)
    skeleton_dataset.set_tails(skeleton.get_tail_ids())
    if args.ckpt_skeleton is not None:
        skeleton_dataset.load(torch.load(
            args.ckpt_skeleton, map_location=device))
    ...
    if args.caster == "sh":
        ...
        if not args.free_opt10:
            skip_rate = -1
        pCaster_origin = BWCaster(
            len(joints), reso_cur_2, device, skip_rate=skip_rate, args=args)
    elif args.caster == "dist":
        pCaster_origin = DistCaster()
    elif args.caster == "mlp":
        if args.free_opt1:
            pCaster_origin = MLPCaster_integrate(
                len(joints), device, args=args)
        elif args.free_opt5:
            pCaster_origin = MLPCaster(
                len(joints), device, args=args, use_interface=False, use_ffmlp=False)
        else:
            pCaster_origin = MLPCaster(
                len(joints), device, args=args, use_ffmlp=False)
    ...
    if args.caster == "sh":
        params = [{'params': grad_vars_sh_field, 'lr': args.lr_sh}]
        if not args.use_gt_skeleton:
            params.append(
                {'params': grad_vars_skeletonpose, 'lr': args.lr_skel})
        ...
        optimizer = torch.optim.Adam(
            [{'params': grad_vars_skeletonpose, 'lr': args.lr_skel}], betas=(0.9, 0.99))
    elif args.caster == "mlp":
        params = [
            {'name': 'weight_nets', 'params': list(
                pCaster_origin.weight_nets.parameters()), 'weight_decay': wd, 'lr': args.lr_mlp},
            {'name': 'encoder', 'params': list(
                pCaster_origin.encoder.parameters()),  'lr': args.lr_mlp * 200}
        ]
        if not args.free_opt5:
            params.append({'name': 'interface_layer', 'params': list(
                pCaster_origin.interface_layer.parameters()), 'weight_decay': wd, 'lr': args.lr_mlp})
        if not args.use_gt_skeleton:
            params.append(
                {'name': 'skeleton', 'params': grad_vars_skeletonpose, 'lr': args.lr_skel})
        optimizer = torch.optim.Adam(params, betas=(0.9, 0.99))

    elif args.caster == "map":
        params = [
            {'name': 'map_nets', 'params': list(
                pCaster_origin.map_nets.parameters()), 'weight_decay': wd, 'lr': 1e-4},
            {'name': 'encoder', 'params': list(
                pCaster_origin.encoder.parameters()),  'lr': 2e-2}
        ]

        params.append({'name': 'pose_params', 'params': list(
            pCaster_origin.pose_params.parameters()), 'weight_decay': wd, 'lr': 1e-4})

        params.append({'name': 'interface_layer', 'params': list(
            pCaster_origin.interface_layer.parameters()), 'weight_decay': wd, 'lr': 1e-4})
        optimizer = torch.optim.Adam(params, betas=(0.9, 0.99))
    elif args.caster == "direct_map":
        wd = 1e-6
        lr = args.lr_mlp
        if args.free_opt11:
            params = [
                {'name': 'mapnets', 'params': list(
                    pCaster_origin.central_map.parameters()), 'weight_decay': wd, 'lr': lr}
            ]
        else:
            params = [
                {'name': 'mapnets', 'params': list(
                    pCaster_origin.map_nets.parameters()), 'weight_decay': wd, 'lr': lr},
                {'name': 'encoder', 'params': list(
                    pCaster_origin.encoder.parameters()),  'lr': 2e-2}
            ]
            params.append({'name': 'pose_params', 'params': list(
                pCaster_origin.pose_params.parameters()), 'weight_decay': wd, 'lr': lr})
            params.append({'name': 'interface_layer', 'params': list(
                pCaster_origin.interface_layer.parameters()), 'weight_decay': wd, 'lr': lr})
            params.append({'name': 'branch_w', 'params': list(
                pCaster_origin.branch_w.parameters()), 'weight_decay': wd, 'lr': lr})
            params.append({'name': 'branch_v', 'params': list(
                pCaster_origin.branch_v.parameters()), 'weight_decay': wd, 'lr': lr})
        optimizer = torch.optim.Adam(params, betas=(0.9, 0.99))
    elif args.caster == "forward":
        optimizer = torch.optim.Adam(
            [{'params': grad_vars_skeletonpose, 'lr': args.lr_skel}], betas=(0.9, 0.99))
            ...
            with torch.no_grad():
                if (iteration % args.vis_every == args.vis_every - 1 and args.N_vis != 0):
                    if rank == rank_criteria:
                        skeleton_dataset.save(
                            f'{logfolder}/{args.expname}_skeleton_it{iteration}.th')
                        sh_field.save(
                            f'{logfolder}/{args.expname}_sh_it{iteration}.th')
                        if not args.caster == "sh":
                            pCaster_origin.save(
                                f'{logfolder}/{args.expname}_pCaster_it{iteration}.th')
                        tensorf.save(
                            f'{logfolder}/{args.expname}_it{iteration}.th')
                        skeleton_props = {"skeleton_dataset": skeleton_dataset}
                        PSNRs_test = evaluation(test_dataset, tensorf, args, renderer, f'{logfolder}/imgs_vis/', N_vis=args.N_vis, prtx=f'{iteration:06d}_',
                                                N_samples=-1, white_bg=white_bg, ndc_ray=ndc_ray, compute_extra_metrics=False, skeleton_props=skeleton_props, device=device)
        ...
        with torch.cuda.amp.autocast():
            ...
            if batching:
                ...
                if False:
                    ...
                    if itr == 0:
                        dataloader_iterator = iter(train_dataloader)
                    data = next(dataloader_iterator)
                    rgb, rays = data['rgbs'].view(-1,
                                                  3), data['rays'].view(-1, 6)
                    idx = torch.randint(0, ray_per_img, size=[
                                        args.batch_size], device=device)
                    # rays_train = torch.index_select(allrays[itr+num_frames*rank_diff].to(device), 0, idx)
                    rgb_train = torch.index_select(
                        rgb.to(device, non_blocking=True), 0, idx)
                    rays_train = torch.index_select(
                        rays.to(device, non_blocking=True), 0, idx)
                    ...
                    # rays_train = torch.index_select(allrays[itr+num_frames*rank_diff].to(device), 0, idx)
                    rgb_train = torch.index_select(
                        allrgbs[itr+num_frames*rank_diff].to(device), 0, idx)
                    rays_train = torch.index_select(
                        allrays[itr+num_frames*rank_diff], 0, idx)
                    # rgb_train = torch.index_select(allrgbs[itr+num_frames*rank_diff], 0, idx)

            if args.use_gt_skeleton:
                skel = test_dataset.frame_skeleton_pose[itr +
                                                        num_frames*rank_diff]
            else:
                skel = skeleton_dataset(
                    allanimframes[itr+num_frames*rank_diff])
            ...
            for i_batch in range(batch_len):
                if batching:
                    ...
                    idx = rand_idxs[i_batch *
                                    args.batch_size:(i_batch+1)*args.batch_size]
                    # rays_train = torch.index_select(allrays[itr+num_frames*rank_diff].to(device), 0, idx)
                    rgb_train = torch.index_select(
                        allrgbs[itr+num_frames*rank_diff].to(device), 0, idx)
                    rays_train = torch.index_select(
                ...
                with torch.cuda.amp.autocast():
                    if args.free_opt2:
                        tensorf.set_tmp_animframe_index(
                            allanimframes[itr+num_frames*rank_diff])
                    ...
                    if args.caster == "grid_map":
                        ...
                        raw_sigma = torch.index_select(
                            tensorf.raw_sigma.view(-1), 0, idx)
                        eloss = eloss * torch.clamp(raw_sigma, 0.0)
                        eloss = eloss.sum(-1).mean() * 0.01
                        total_loss += eloss

                        pbar_description = f'Iteration {iteration:05d}:' + f' mse = {loss:.4f}' + \
                            f' tvloss = {tvloss:.4f}' + f' eloss = {eloss:.4f}' + \
                    ...
                    if args.caster == "direct_map":
                        ...
                        if args.free_opt11:
                            num_sample = 1000
                            loss1, loss2 = pCaster_origin.central_loss(
                                num_sample=num_sample)
                            total_loss = loss1 * 0.01
                            total_loss += loss2 * 0.01
                            pbar_description = f'Iteration {iteration:05d}:' + \
                                f' mse = {loss:.4f}' + f' loss1 = {loss1:.4f}' + \
                                f' loss2 = {loss2:.4f}'
                        else:
                            eloss = 0
                            eloss, idx = pCaster_origin.compute_elastic_loss(
                                num_sample=num_sample, scale=0.08)
                            raw_sigma = torch.index_select(
                                tensorf.raw_sigma.view(-1), 0, idx)
                            eloss = eloss * torch.clamp(raw_sigma, 0.0)
                            eloss = eloss.sum(-1).mean() * 0.01
                    ...
                    if args.caster == "bwf":
                        ...
                        pbar_description = f'Iteration {iteration:05d}:' + \
                            f' mse = {loss:.4f}' + f' tvloss = {tvloss:.4f}'
                    ...
                    if args.caster == "mlp":
                        num_sample = 10000
                        eloss = 0
                        eloss, idx = pCaster_origin.compute_weight_elastic_loss(
                            num_sample=num_sample)
                        raw_sigma = torch.index_select(
                            tensorf.raw_sigma.view(-1), 0, idx)
                        eloss = eloss * torch.clamp(raw_sigma, 0.0)
                        ...
                        # w_sum, sigma = tensorf.occupancy.view(-1), tensorf.weights_sum.view(-1)
                        # w_sum = clip_weight(w_sum, thresh = 1e-6)
                        # sigma = clip_weight(sigma, thresh = 0.01)

                        # sigma_loss = torch.mean((w_sum - sigma)**2) * 0.01

                        jpos = skeleton.get_listed_positions()
                        parent_ids = torch.tensor(
                            skeleton.get_listed_parents()).to(jpos.device)
                        jpos_p = torch.index_select(jpos, 0, parent_ids)
                        jpos = (jpos + jpos_p) / 2
                        j_weights = pCaster_origin.compute_weights(
                            jpos, pCaster_origin.cache_transforms)
                        j_weights_sum = torch.sum(j_weights, dim=1)
                        j_weights_sum = torch.clamp(j_weights_sum, min=1e-6)
                        j_weights = j_weights / j_weights_sum.unsqueeze(1)
                        j_w_loss = j_weights - \
                            torch.eye(
                                j_weights.shape[0], device=j_weights.device)
                        j_w_loss = torch.mean(j_w_loss ** 2)
                        total_loss += j_w_loss * 1

                        # total_loss += eloss

                        # total_loss += sigma_loss

                        # total_loss += tvloss  * 0.1

                        pbar_description = f'Iteration {iteration:05d}:' + f' mse = {loss:.4f}' + \
                            f' elastic_loss = {eloss:.4f}' + \
                            f' central_loss = {j_w_loss:.4f}'
                    ...
                    if False:
                        ...
                        if TV_weight_density > 0:
                            ...
                            loss_tv = tensorf.TV_loss_density(
                                tvreg) * TV_weight_density
                        ...
                        if TV_weight_app > 0:
                            ...
                            loss_tv = loss_tv + \
                                tensorf.TV_loss_app(tvreg)*TV_weight_app
                            total_loss = total_loss + loss_tv
                            summary_writer.add_scalar(
