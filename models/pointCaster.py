...
class MLPCaster(CasterBase):
    ...
    def compute_weights_grad(self, points):
        if not self.args.free_opt8:
            ...
            tmp = functorch.vmap(functorch.jacfwd(func), in_dims = 0, out_dims = 0)(encoded_pos.view(-1, 1,self.interface_dim)).squeeze(1)
            tmp = torch.matmul(tmp, encoded_pos.view(-1, self.interface_dim, 1)).squeeze(-1)
            return tmp
...
class BWCaster(CasterBase):
    ...
    def get_optparam_groups(self, lr_init_spatialxyz = 0.02, lr_init_network = 0.001, wd = 1e-6):
        grad_vars = [{'params': self.app_line, 'lr': lr_init_spatialxyz, 'weight_decay': wd}, {'params': self.app_plane, 'lr': lr_init_spatialxyz, 'weight_decay': wd}]
        return grad_vars
    ...
    def init_one_svd(self, n_component, gridSize, scale, device):
        ...
        for i in range(len(self.vecMode)):
            ...
            line_coef.append(
                torch.nn.Parameter(
                   -(scale * 0.5) +  0.1 + scale * torch.randn((self.j_channel, 1, n_component[i], gridSize[vec_id], 1))
                    ))
    ...
    def sample_BWfield(self, xyz_sampled):
        ...
        for i in range(xyz_sampled.shape[0]):
            coordinate_plane = torch.stack((xyz_sampled[i, ..., self.matMode[0]], xyz_sampled[i,..., self.matMode[1]], xyz_sampled[i,..., self.matMode[2]])).view(3, -1, 1, 2)
