...
class TensoRFnGP(TensorBase):
    ...
    def get_optparam_groups(self, lr_init_spatialxyz = 0.02, lr_init_network = 0.001):
        grad_vars = [
        {'name': 'encoding', 'params': list(self.encoder.parameters()), 'lr': 1e-16},
        {'name': 'net', 'params': list(self.sigma_net.parameters()) + list(self.color_net.parameters()), 'weight_decay': 1e-6, 'lr': 1e-3}
        ]
        return grad_vars
    ...
    def forward(self, rays_chunk, white_bg=True, is_train=False, ndc_ray=False, N_samples=-1, skeleton_props=None, is_render_only=False):        
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            if torch.isnan(xyz_sampled).any(): 
                raise ValueError('yzs nan')
            ...
            if use_ngp_code:
                ...
                alphas, weights, bg_weight = raw2alpha(sigma, dists * self.distance_scale)
            ...
            # #Todo: clamp
            # second_xyzs = clamp_xyz(second_xyzs, self.bound, self.bound_rate, self.bound_box_rate)
            ...
            if torch.isnan(density_outputs['sigma']).any():
                raise ValueError('density nan')
            # if use_ngp_code:
            ...
            if torch.isnan(deltas).any():
                raise ValueError('delta nan')
            if torch.isnan(alphas).any():
                raise ValueError('alpha nan')
            if torch.isnan(weight).any():
                raise ValueError('weight nan')
            # else:
            #     alphas, weights, bg_weight = raw2alpha(sigma, dists * self.distance_scale)
