...
# TensorVM, TensorCP, raw2alpha, TensorVMSplit, AlphaGridMask
...
def crop_rays(ray, w, h, crop_box, rgb=None, filter=1):
    ...
    # print(ray.shape)
    ray = ray[crop_box[0][0]:crop_box[0][1]:filter,
              crop_box[1][0]:crop_box[1][1]:filter, :]
    # print(ray.shape)
    ray = ray.reshape(-1, 6)
    # print(ray.shape)
    if rgb is not None:
...
def OctreeRender_trilinear_fast(rays, tensorf, chunk=4096, N_samples=-1, ndc_ray=False, white_bg=True, is_train=False, device='cuda', skeleton_props=None, is_render_only=False, render_weights=False):
    ...
    for chunk_idx in range(N_rays_all // chunk + int(N_rays_all % chunk > 0)):
        ...
        rgb_map, depth_map = tensorf(rays_chunk, is_train=is_train, white_bg=white_bg, ndc_ray=ndc_ray,
                                     N_samples=N_samples, skeleton_props=skeleton_props, is_render_only=is_render_only)
...
def evaluation(test_dataset, tensorf, args, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    # exit("exit_test")
    idxs = list(
        range(n_vis_offset, test_dataset.all_rays.shape[0], img_eval_interval))
    ...
    for idx, samples in tqdm(enumerate(test_dataset.all_rays[n_vis_offset::img_eval_interval].to(device)), file=sys.stdout):
        ...
        if not args.data_preparation:
            ...
            if skeleton_dataset is not None and not args.use_gt_skeleton:
                ...
                if args.free_opt4:
                    tensorf.skeleton.rotations_to_transforms_fast(
                        gt_skeleton_pose, type=args.pose_type)
                    ...
                    rotations = tensorf.skeleton.matrix_to_euler_pos_batch(
                        tfs[..., :3, :3], top_batching=True)
                    gt_skeleton_pose = torch.cat(
                        [rotations, translates], dim=-1)
        ...
        rgb_map, depth_map = rgb_map.reshape(
            H, W, 3).cpu(), depth_map.reshape(H, W).cpu()
        ...
        if len(test_dataset.all_rgbs) and test_evaluation:
            ...
            if compute_extra_metrics:
                ...
                l_a = rgb_lpips(gt_rgb.numpy(), rgb_map.numpy(),
                                'alex', tensorf.device)
                l_v = rgb_lpips(gt_rgb.numpy(), rgb_map.numpy(),
                                'vgg', tensorf.device)
                ssims.append(ssim)
                l_alex.append(l_a)
                l_vgg.append(l_v)
        ...
        if not args.free_opt9:
            gt_rgb = (test_dataset.all_rgbs[idxs[idx]].view(
                H, W, 3).numpy() * 255).astype('uint8')
            # gt_rgb = (test_dataset.all_rgbs[n_vis_offset].view(H, W, 3).numpy() * 255).astype('uint8')
            # rgb_map = np.concatenate((rgb_map, gt_rgb), axis=1)

        else:
            gt_rgb = (test_dataset.all_rgbs[idxs[idx]].view(gt_h, gt_w, 3)[
                      ::10, ::10, :].numpy() * 255).astype('uint8')
        rgb_map = np.concatenate((rgb_map, gt_rgb), axis=1)
        # tmp
        # if not args.data_preparation:
        ...
        if args.free_opt9:
            names = tensorf.skeleton.get_listed_names()
            for i in range(render_weights.shape[-1]):
                imageio.imwrite(f'{savePath}/weights_{prtx}{idx:02d}_{i:02d}_{names[i]}.png', (
                    render_weights[..., i].reshape(H, W, 1).cpu().numpy() * 255).astype('uint8'))
...
def evaluation_path(test_dataset, tensorf, c2ws, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    for idx, c2w in tqdm(enumerate(c2ws)):
        ...
        rays_o, rays_d = get_rays(
            test_dataset.directions, c2w)  # both (h*w, 3)
        if ndc_ray:
    ...
    imageio.mimwrite(f'{savePath}/{prtx}video.mp4',
                     np.stack(rgb_maps), fps=30, quality=8)
    imageio.mimwrite(f'{savePath}/{prtx}depthvideo.mp4',
                     np.stack(depth_maps), fps=30, quality=8)
